...
- queue/dead-letter 수동 운영 API는 `admin` 이상(role hierarchy: `owner > admin > member > viewer`)만 접근 가능
...
- 플랫폼 JWT 클레임: `tenant_id`, `sub`(membership user id), `role`, `iss`, `aud`, `exp`
- 플랫폼 서명 키:
...
- request-path DB direct read: `DATA_BACKEND=postgres`일 때 tenant-scoped PostgreSQL direct query를 우선 사용
- queue/dead-letter/retry snapshot 저장은 단일 PostgreSQL transaction으로 수행
- 비정상 종료 시 `running` 상태 스캔은 startup recovery에서 자동 재개
