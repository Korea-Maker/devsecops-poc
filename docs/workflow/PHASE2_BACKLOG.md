...
| 타입체크 통과 | `pnpm --filter @previo/api typecheck` 0 errors |
...
| 타입 안전 | `pnpm --filter @previo/api typecheck` 0 errors |
...
| 커버리지 | `pnpm --filter @previo/api typecheck` 0 errors |
...
| 타입체크 | `pnpm --filter @previo/api typecheck` 0 errors |
...
[ ] 전체 typecheck: pnpm --filter @previo/api typecheck 0 errors
[ ] 전체 테스트: pnpm --filter @previo/api test 통과
[ ] 빌드: pnpm --filter @previo/api build 성공
```
...
> 업데이트: 2026-02-27
> 목적: 워커 중지/재시도 타이머 정책 명시화와 source-prep 실패 관측성 계약을 코드/테스트/문서에 반영

### 완료 체크리스트

- [x] 워커 중지 정책 명시화 (`apps/api/src/scanner/queue.ts`)
  - [x] `stopScanWorker()` 호출 시 워커 interval 중지 + pending retry timer 전량 취소
  - [x] stop 이후 retry timer 잔존으로 인한 예기치 않은 재enqueue 방지 정책을 주석/코드로 고정
  - [x] `getPendingRetryTimerCount()` 보조 함수 추가로 정책 검증 가능성 확보
- [x] cleanup/source-prep 실패 관측성 강화
  - [x] queue `finally` cleanup 실패 시 `[scanner-queue]` prefix `console.warn` 로그 출력
  - [x] source-prep clone 실패 경로의 cleanup 실패도 `[scanner-queue]` prefix warn 로그로 기록
  - [x] source-prep 구조화 에러 계약 추가
    - [x] `SOURCE_PREP_CLONE_FAILED`
    - [x] `SOURCE_PREP_UNSUPPORTED_REPO_URL`
    - [x] `SourcePrepError`/`isSourcePrepError`로 테스트 가능한 타입 계약 제공
- [x] 테스트 보강
  - [x] `stopScanWorker()`의 retry timer 취소 정책 검증
  - [x] cleanup 실패 warn 관측 신호(`console.warn` spy) 검증
  - [x] source-prep unsupported/clone 실패 구조화 에러 코드 계약 검증
- [x] 문서 동기화
  - [x] README 워커 중지/재시도 타이머 정책 반영

## Phase 2-9 완료 항목

> 업데이트: 2026-02-27
> 목적: Phase 2 마감 정리(운영 가시성 API + 응답 계약 통일 + 테스트/문서 동기화)

### 완료 체크리스트

- [x] 운영 가시성 API 추가 (`apps/api/src/routes/scans.ts`)
  - [x] `GET /api/v1/scans/queue/status` 구현
    - [x] 응답: `{ queuedJobs, deadLetters, pendingRetryTimers, workerRunning, processing }`
  - [x] `POST /api/v1/scans/queue/process-next` 구현
    - [x] 응답: `{ processed: boolean, busy: boolean }`
    - [x] `busy`로 `empty` vs `in-flight` 상태 구분 가능
- [x] 큐 상태 보조 함수/계약 정비 (`apps/api/src/scanner/queue.ts`)
  - [x] `isScanWorkerRunning()`
  - [x] `getQueueStatus()`
  - [x] `processNextScanJob()` 반환 계약을 `{ processed, busy }`로 확장
- [x] scans 라우트 에러 응답 shape 통일 + 4xx 보존
  - [x] 공통 shape: `{ error, code? }`
  - [x] `setErrorHandler`에서 client error(4xx)는 status를 보존하고 shape만 통일
  - [x] 내부 서버 오류만 `500 + SCAN_ROUTE_INTERNAL_ERROR`로 정규화
  - [x] 주요 코드 노출: `SCAN_INVALID_ENGINE`, `SCAN_INVALID_REPO_URL`, `SCAN_NOT_FOUND`, `DEAD_LETTER_*`
- [x] 스캔 실패 코드 노출 강화 + 타입 계약 통일
  - [x] `ScanRecord.lastErrorCode` 필드 추가
  - [x] queue 처리 실패 시 `lastErrorCode` 저장/조회 가능
  - [x] `ScanErrorCode` 공유 union 타입 도입으로 queue/store 타입 정합성 확보
  - [x] source-prep 구조화 에러 코드는 `SOURCE_PREP_*` 그대로 유지 가능
- [x] 테스트 보강 (`apps/api/tests/scans.test.ts`, `apps/api/tests/queue.test.ts`)
  - [x] queue status 기본값 + `processing` 상태 검증
  - [x] process-next `empty/busy/processed` 케이스 검증
  - [x] invalid JSON 요청의 4xx 보존 검증
  - [x] 에러 코드/응답 shape 회귀 검증 보강
- [x] README 운영 API 사용법 반영

---

## Phase 2 구현됨 vs 다음 Phase TODO

### ✅ Phase 2에서 구현됨

- in-memory scan store + queue worker + retry/backoff + dead-letter + redrive
- mock/native 실행 모드 분기 (`SCAN_EXECUTION_MODE`)
- native source-prep (로컬 경로 passthrough + 원격 shallow clone + cleanup)
- findings 요약 저장 및 조회 API
- 운영/관리 API
  - queue 상태 조회 (`/queue/status`, `processing` 포함)
  - 즉시 1건 처리 트리거 (`/queue/process-next`, `busy` 신호 포함)
- scans 라우트 에러 응답 shape 통일 (`{ error, code? }`) + 4xx 상태 보존

### ⏭️ 다음 Phase TODO (Phase 3+)

- 영속 큐/저장소 전환 (Redis/Broker/DB 기반)
- 스캔 상세 결과(파일/라인 단위) 저장 모델 + 조회 API
- native 파서 정밀도/버전 호환성 고도화
- source-prep 캐시/TTL/용량 정책 및 운영 메트릭 체계화
- 권한/인증/감사 로그 기반 운영 제어(관리 API 접근 통제)
