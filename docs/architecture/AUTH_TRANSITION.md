...
- OIDC callback 기반 플랫폼 토큰 발급
  - `GET /api/v1/auth/jwks`
  - `GET /api/v1/auth/google/start`
  - `GET /api/v1/auth/google/callback`
  - Google `id_token` 검증 후 membership 매핑 + 자체 RS256 JWT 발급

### JWT claim 매핑 계약

- 모든 필드는 required이며, env로 primary/fallback selector를 오버라이드할 수 있다.
- 기본값(하위호환):
  - `tenantId`: `tenant_id` (fallback `tid`)
  - `userId`: `sub` (fallback `user_id`)
  - `role`: `role` (fallback `roles[0]`)
- role 허용값: `owner | admin | member | viewer`
- selector 형식: `<claim>` 또는 `<claim>[n]`
- fallback env가 빈 문자열이면 fallback 비활성화
...
- `JWT_TENANT_ID_CLAIM` (기본 `tenant_id`)
- `JWT_TENANT_ID_FALLBACK_CLAIMS` (기본 `tid`, CSV)
- `JWT_USER_ID_CLAIM` (기본 `sub`)
- `JWT_USER_ID_FALLBACK_CLAIMS` (기본 `user_id`, CSV)
- `JWT_ROLE_CLAIM` (기본 `role`)
- `JWT_ROLE_FALLBACK_CLAIMS` (기본 `roles[0]`, CSV)

유효성 규칙:

- 필수값 누락 시 `503 TENANT_AUTH_JWT_CONFIG_INCOMPLETE`
- `JWT_JWKS_URL` 형식 오류 시 `503 TENANT_AUTH_JWT_CONFIG_INVALID`
- claim selector 형식 오류/빈 primary 설정 시 `503 TENANT_AUTH_JWT_CLAIM_MAPPING_INVALID`
...
- 설정/JWKS/claim 매핑
  ...
  - `TENANT_AUTH_JWT_CLAIM_MAPPING_INVALID` (503)
...
- access token claim 스키마를 기본 계약(tenant_id/tid, sub/user_id, role/roles[0]) 또는 설정된 env 매핑과 정렬
...
- [x] API OIDC callback + 플랫폼 JWT 발급/JWKS 엔드포인트 구현
- [ ] OAuth 로그인/토큰 발급 플로우(웹 화면/세션) 연결
