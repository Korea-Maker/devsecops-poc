...
Previo — 스타트업 개발팀(1~5명)을 위한 DevSecOps 플랫폼. SAST(Semgrep) + SCA(Trivy) + Secret(Gitleaks) 스캔을 통합하고, 대시보드와 GitHub CI/CD 연동을 제공한다.
...
# 단일 테스트 파일 실행
pnpm --filter @previo/api exec vitest run tests/scans.test.ts

# 테스트 워치 모드
pnpm --filter @previo/api test:watch
